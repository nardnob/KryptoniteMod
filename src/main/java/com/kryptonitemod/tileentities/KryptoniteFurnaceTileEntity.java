...
public class KryptoniteFurnaceTileEntity extends TileEntity implements ITickableTileEntity, INamedContainerProvider {
    public static final String NAME = "kryptonite_furnace_tile_entity";
    ...
    public final ItemStackHandler inventory = new ItemStackHandler(3) {
        ...
        protected void onContentsChanged(final int slot) {
            ...
            KryptoniteFurnaceTileEntity.this.markDirty();
        }
    };

    public KryptoniteFurnaceTileEntity() {
        super(KryptoniteTileEntityTypes.KRYPTONITE_FURNACE.get());
    }
    ...
    public void tick() {
        ...
        if (lastBurning != hasFuel) { // We use hasFuel because the current fuel may be all burnt out but we have more that will be used next tick
            ...
            final BlockState newState = this.getBlockState()
                    .with(KryptoniteFurnaceBlock.BURNING, hasFuel);
    ...
    public ITextComponent getDisplayName() {
        return new TranslationTextComponent(KryptoniteBlocks.KRYPTONITE_FURNACE.get().getTranslationKey());
    }

    /**
     * Called from {@link NetworkHooks#openGui}
     * (which is called from {@link KryptoniteFurnaceBlock#onBlockActivated} on the logical server)
     *
    ...
    public Container createMenu(final int windowId, final PlayerInventory inventory, final PlayerEntity player) {
        return new KryptoniteFurnaceContainer(windowId, inventory, this);
    }
}